...
const SingleBlog = () => {
    const { id } = useParams();
    const { userId, user, navigate, base_url } = useGlobalContext();
    ...
    const handleLike = async () => {
        try {
            if (!liked) {
                ...
                await axios.post(`${base_url}/api/blog/likeBlog/${id}`, {
                    withCredentials: true
                });
            } else {
                ...
                await axios.post(`${base_url}/api/blog/removelikeBlog/${id}`, {
                    withCredentials: true
                });
    ...
    const handleComment = async () => {
        if (newComment.trim() !== "") {
            ...
            await axios.post(`${base_url}/api/blog/commentBlog/${id}`, { comment: newComment }
                , {
                    withCredentials: true
                });
            setNewComment("");
                ...
                {Object.keys(data).length > 0 && (
                    <>
                        ...
                        <div className="flex items-center space-x-4 pb-2 pl-1 ">
                            <button onClick={handleLike} className="flex items-center text-gray-600 hover:text-gray-800">
